        ...
        ZStack {
            ...
            NavigationView {
                if viewData.datesAssignments.count > 0 {
                    List {
                        TodaySectionView(date: self.today, viewData: viewData)
                        
                        ForEach(Array(viewData.datesAssignments.keys.sorted(by: <)), id: \.self) { date in
                            if date >= self.today {
                                SectionView(date: date, today: self.today, viewData: self.viewData)
                            }
                        }
                    }
                    .onAppear{
    ...
    var body: some View {
        VStack(alignment: .leading) {
            HStack {
                ...
                ? Text("Submitted").font(.system(size: 12)).foregroundColor(.green)
                : Text("Not Submitted").font(.system(size: 12)).foregroundColor(.red)
            }
            HStack {
